...
class ProxyManager {
  ...
  async fetchProxiesFast() {
    console.log('[JDProxy] 获取代理列表（13个源并发）...');
    const allProxies = new Set();

    // 13个代理源（与1688/亚马逊方案一致）
    const sources = [
    ...
    console.log(`[JDProxy] 获取到 ${allProxies.size} 个代理 (成功源:${successSources}/13, HTTP:${httpCount}, SOCKS:${socksCount})`);
  ...
  async refreshProxies(force = false) {
    ...
    // 防止并发刷新
    if (this.refreshing) {
      console.log('[JDProxy] 刷新进行中，跳过本次请求');
      return this.proxies;
    }
    ...
    try {
      ...
      console.log(`[JDProxy] 刷新完成: ${finalList.length} 个代理 (已知好:${this.knownGoodProxies.length})`);
      return finalList;
    } catch (e) {
      console.error('[JDProxy] 刷新失败:', e.message);
      this.proxies = [...this.knownGoodProxies];
      return this.proxies;
    } finally {
  ...
  startAutoRefresh(intervalMinutes = 30) {
    ...
    console.log(`[JDProxy] 启动自动刷新定时器：每${intervalMinutes}分钟刷新一次`);

    this.autoRefreshTimer = setInterval(async () => {
      if (!this.enabled) {
        console.log('[JDProxy] 代理已禁用，跳过自动刷新');
        return;
      }
      try {
        console.log(`[JDProxy] 自动刷新触发 ${new Date().toISOString()}`);
        const before = this.proxies.length;
        const after = await this.refreshProxies(true);
        console.log(`[JDProxy] 自动刷新完成: ${before} → ${after.length} 个代理`);
      } catch (e) {
        console.error('[JDProxy] 自动刷新异常:', e.message);
      }
  ...
  stopAutoRefresh() {
    if (this.autoRefreshTimer) {
      ...
      console.log('[JDProxy] 已停止自动刷新定时器');
    }
  }
  ...
  markFailed(proxy) {
    ...
    if (isKnownGood) {
      ...
      if (failCount >= 3) {
        ...
        console.log(`[JDProxy] 已知代理 ${proxy} 失败3次，暂时跳过`);
      } else {
  ...
  markSuccess(proxy) {
    ...
    if (!this.knownGoodProxies.includes(proxy) && this.knownGoodProxies.length < 20) {
      ...
      console.log(`[JDProxy] 代理 ${proxy} 成功，加入已知好代理列表 (共${this.knownGoodProxies.length}个)`);
    }
  }
